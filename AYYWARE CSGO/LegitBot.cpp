...
void CLegitBot::legitbacktrack(CUserCmd *pCmd)
{
	...
	if (Menu::Window.LegitBotTab.LegitBackTracking.GetState())
	{
		...
		for (int i = 0; i < Interfaces::Engine->GetMaxClients(); i++)
		{
			...
			if (entity->IsAlive())
			{
				...
				float FOVDistance = distance_point_to_line(hitboxPos, pLocal->GetEyePosition(), pCmd->viewangles + (pLocal->localPlayerExclusive()->GetAimPunchAngle() * 2.f));
		...
		if (bestTargetIndex != -1)
		{
			...
			for (int t = 0; t < 12; ++t)
			{
				float tempFOVDistance = distance_point_to_line(headPositions[bestTargetIndex][t].hitboxPos, pLocal->GetEyePosition(), pCmd->viewangles + (pLocal->localPlayerExclusive()->GetAimPunchAngle() * 2.f));
...
void CLegitBot::SyncWeaponSettings()
{
	...
	{
		Speed = Menu::Window.LegitBotTab.WeaponPistSpeed.GetValue() / 50;
	...
	{
		Speed = Menu::Window.LegitBotTab.WeaponSnipSpeed.GetValue() / 50;
		...
		{
			switch (Menu::Window.LegitBotTab.WeaponSnipScoutHitbox.GetIndex())
			{
			case 0:
				HitBox = ((int)CSGOHitboxID::Head);
				break;
	...
	{
		Speed = Menu::Window.LegitBotTab.WeaponMainSpeed.GetValue() / 50;
...
void CLegitBot::DoAimbot(CUserCmd *pCmd, bool &bSendPacket)
{
	...
	if (TargetID >= 0 && pTarget)
	{
		...
		if (GameUtils::IsPistol(pWeapon))
		{
			int Key = Menu::Window.LegitBotTab.WeaponPistKey.GetKey();
		...
		{
			int Key = Menu::Window.LegitBotTab.WeaponSnipKey.GetKey();
			if (Key >= 0 && !GUI.GetKeyState(Key))
			{
				TargetID = -1;
				pTarget = nullptr;
		...
		{
			int Key = Menu::Window.LegitBotTab.WeaponMainKey.GetKey();
			if (Key >= 0 && !GUI.GetKeyState(Key))
			{
				TargetID = -1;
				pTarget = nullptr;
