...
void CTextField::Draw(bool hover)
{
	...
	Render::Clear(a.x + 2, a.y + 2, m_iWidth - 4, 12, Color(129, 129, 129, 0));
	if (hover || IsGettingKey)
		Render::Clear(a.x + 2, a.y + 2, m_iWidth - 4, 12, Color(129, 129, 129, 0));

	const char *cstr = text.c_str();

	Render::Text(a.x + 2, a.y + 2 - 1, Color(255, 255, 255, 255), Render::Fonts::Menu, cstr);
}
...
void CSlider::Draw(bool hover)
{
	...
	Render::Clear(a.x - 1.5, a.y + 5, m_iWidth, 9, Color(129, 129, 129, 0));
	Render::Clear(a.x - 1.5, a.y + 5, Location, 9, Color(Menu::Window.ColorTab.MenuInnerR.GetValue(), Menu::Window.ColorTab.MenuInnerG.GetValue(), Menu::Window.ColorTab.MenuInnerB.GetValue(), 255));
	Render::Outline(a.x - 1.5, a.y + 5, m_iWidth, 9, Color(255, 255, 255, 255));


	char buffer[24];
	sprintf_s(buffer, "%.2f", Value);
	RECT txtSize = Render::GetTextSize(Render::Fonts::MenuBold, buffer);
	Render::Text(a.x - 1.5 + Location - txtSize.right / 2, a.y + 14, Color(255, 255, 255, 255), Render::Fonts::MenuBold2, buffer);
}
...
void CListBox::Draw(bool hover)
{
	...
	if (Items.size() > 0)
	{
		...
		for (int i = ScrollTop; (i < Items.size() && drawnItems < ItemsToDraw); i++)
		{
			...
			RECT ItemRegion = { a.x + 1, a.y + 1 + drawnItems * 16, m_iWidth - 2 , 16 };

			Render::Text(ItemRegion.left + 4, ItemRegion.top + 2, Color(255, 255, 255, 255), Render::Fonts::MenuText, Items[i].c_str());
			drawnItems++;
		}
		...
		Render::Clear(a.x + m_iWidth, a.y + posRatio, 4, sizeRatio, UI_COL_TABTEXT);
	}
