...
void __stdcall Hooked__PlaySoundCSGO(const char* fileName)
{
	...
	if (Interfaces::Engine->IsInGame() || !Menu::Window.SettingsTab.OtherSettingsAutoAccept.GetState())
		return;
...
void Hacks::DrawHacks()
{
	...
	bool sss = true;
	int SkinToggle = Menu::Window.SkinchangerTab.OtherSkinToggle.GetKey();
	if (SkinToggle > 0 && GUI.GetKeyState(SkinToggle))
	{
		while (sss) {
			if (Menu::Window.SkinchangerTab.SkinEnable.GetState())
			{
				Menu::Window.SkinchangerTab.SkinEnable.SetState(false);
				sss = !sss;
				Sleep(100);
			}
			else
			{
				Menu::Window.SkinchangerTab.SkinEnable.SetState(true);
				sss = !sss;
				Sleep(100);
			}
...
void Hacks::MoveHacks(CUserCmd *pCmd,bool &bSendPacket)
{
	...
	int SkinApply = Menu::Window.SkinchangerTab.OtherSkinApply.GetKey();
	if (SkinApply > 0 && GUI.GetKeyState(SkinApply))
	{
		static ConVar* Meme = Interfaces::CVar->FindVar("cl_fullupdate");
		Meme->nFlags &= ~FCVAR_CHEAT;
		Interfaces::Engine->ClientCmd_Unrestricted("cl_fullupdate");
	...
	if (Menu::Window.SettingsTab.OtherSettingsMemeWalk.GetState())
	{
	...
	{
		Interfaces::Engine->ClientCmd_Unrestricted("bind w +forward; bind s +back; bind a +moveleft; bind d +moveright");
	}
	int FakeWalkKey = Menu::Window.MiscTab.OtherFakeWalk.GetKey();
